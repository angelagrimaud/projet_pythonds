...
L'objectif de ce projet est donc de proposer un modèle permettant de prédire, en utilisant à la fois des données historiques sur la consommation d'électricité, et des données météorologiques, la variation à court terme (*ie* sur des durées de l'ordre de l'heure) de la consommation d'électricité. 
